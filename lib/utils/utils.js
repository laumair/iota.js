...
**/
var transactionObject = function (trytes, options) {
    ...
    // If hash is provided, do not generate the transaction hash
    if (inputValidator.isObject(options) && inputValidator.isHash(options.hash)) {
        thisTransaction.hash = options.hash;
    } else {
        var hash = [];
