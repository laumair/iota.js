...
function minimalSignatureOverlap(normalizedBundleHashes, security, tritSecurityThreshold) {
    var overlapMatrix = Array(security * 27).fill(Array(27).fill(0))
    var bundles = normalizedBundleHashes.map(x => x.slice(0, security * 27))

    for (let j = 0; j < bundles.length; j++) {
        for (let k = 0; k < security * 27; k++) {
            for (let l = 0; l < 27; l++) {
                if (bundles[j][k] === l || l === 13) {
                    overlapMatrix[k][l] = 1
                }
            }
        }
    ...
    return overlapMatrix
        ...
        .filter((x) => x === 0).length / 3 >= tritSecurityThreshold
}
...
*
**/
Bundle.prototype.finalize = function (signedNormalizedBundleHashes, security) {
    var validBundle = false;
    var tritSecurityThreshold = 81 * security;
    ...
    while (!validBundle) {
        ...
        if (
            normalizedHash.indexOf(13 /* = M */) != -1 ||
            !minimalSignatureOverlap(
                [normBundleHash, ...signedNormalizedBundleHashes],
                security,
                tritSecurityThreshold
            )
        ) {
